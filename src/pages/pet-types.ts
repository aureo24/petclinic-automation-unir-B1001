...
export class PetType{
    ...
    public readonly addNewPetTypeButton = this.page.locator("[class*='btn btn-default']",{hasText:'Add'});
    public readonly newPetTypeContainer = this.page.locator('[class="form-horizontal ng-untouched ng-pristine ng-invalid"]');

    public async addNewPetType(specialistName:string){
        await this.addNewPetTypeButton.click();
        await (this.newPetTypeContainer).waitFor({state:'visible'});
    ...
    public async editPetType(actualValue:string, newValue:string){
        await this.page.locator('tbody').locator('tr').first().waitFor({state:'visible'})
        const rowIndex = await getRowIndex(this.page,actualValue,"input[name='pettype_name']");
    ...
    public async deletePetType(actualValue:string){
        await this.page.locator('tbody').locator('tr').first().waitFor({state:'visible'})
        const rowIndex = await getRowIndex(this.page,actualValue,"input[name='pettype_name']");
